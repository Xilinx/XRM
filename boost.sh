...
usage()
{
    ...
    echo "% boost.sh -prefix $PWD/tmp/boost"
    echo ""
    echo "Clone boost into $PWD/tmp/boost/build but don't build"
    echo "% boost.sh -prefix $PWD/tmp/boost -nobuild"
    echo ""
    echo "Build boost (from already cloned src) and install under $PWD/tmp/boost/xrm/u16"
    echo "% boost.sh -srcdir $PWD/tmp/boost/build -install $PWD/tmp/boost/xrm/u16"
...
if [[ $nobuild == 0 ]]; then
    ...
    ./b2 -a -d+2 cxxflags="-std=gnu++11 -fPIC" -j6 install --prefix=$install --build-type=complete address-model=64 architecture=x86 link=static threading=multi --with-filesystem --with-program_options --with-system --with-thread --with-serialization --layout=tagged 
